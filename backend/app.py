...
def submit_answer():
    ...
    question_count +=1
    temp_path = ( #converts speech data to text by storing it in temp file
        tempfile.NamedTemporaryFile
        (delete=False , suffix= ".webm")
        ).name
    audio_file.save(temp_path)
    answer = speech_to_text(temp_path)
    os.unlink(temp_path) # deletes the previous stored data to allow app for not losing latency
    if not answer:
